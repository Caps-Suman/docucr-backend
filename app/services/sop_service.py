    ...
    def generate_sop_pdf(sop: Any) -> bytes:
        ...
        def _ext_by_doc(key, filter_fn):
            ...
            for doc in extracted_docs:
                ...
                if items:
                    result.append({"name": doc.get("name", ""), "items": items})
            return result
        ...
        def source_badge(name):
            """Renders a pill like 'Extracted from: filename.pdf' mirroring DocSourceBadge."""
            t = Table(
                [[Paragraph(
                    f'<font color="#1d4ed8"><b>&#9679; Extracted from: {name}</b></font>',
                    ParagraphStyle('SourceBadge', parent=bs, fontSize=9, textColor=colors.HexColor('#1d4ed8'))
                )]],
                colWidths=[7.0*inch]
            )
        ...
        if base_billing or ext_billing:
            ...
            for entry in ext_billing:
                ...
                story.append(source_badge(entry["name"]))
                story.append(Spacer(1, 4))
