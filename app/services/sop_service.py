    ...
    def generate_sop_pdf(sop: Any) -> bytes:
        ...
        def _ext_by_doc(key, filter_fn):
            ...
            for doc in extra_docs:
                items = [x for x in (doc.get(key) or []) if filter_fn(x)]
                if items:
                    result.append({
                        "name": doc.get("name", ""),
        ...
        for doc in extra_docs:
            for group in (doc.get('billing_guidelines') or []):
                _add_billing_group(group)
        ...
        for pg in (sop.get('payer_guidelines') or []):
            _add_payer(pg)
        for doc in extra_docs:
            for pg in (doc.get('payer_guidelines') or []):
                _add_payer(pg)
        ...
        for doc in extra_docs:
            for r in (doc.get('coding_rules_cpt') or []):
                k = (r.get('cptCode',''), r.get('ndcCode',''))
                if k not in seen_cpt_keys:
                    seen_cpt_keys.add(k)
                    all_cpt_rows.append(r)
        ...
        for doc in extra_docs:
            for r in (doc.get('coding_rules_icd') or []):
                k = r.get('icdCode','')
                if k not in seen_icd_keys:
                    seen_icd_keys.add(k)
                    all_icd_rows.append(r)
