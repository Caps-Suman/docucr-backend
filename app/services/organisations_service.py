...
class OrganisationService:
    ...
    def create_organisation(org_data: Dict, db: Session) -> Dict:
        ...
        # 2️⃣ Create Admin User (belongs to this org)
        admin_user = User(
            id=str(uuid.uuid4()),
            email=org_data["email"],
            username=org_data["username"],
            ...
            phone_number=org_data.get("phone_number"),
            is_superuser=False,
            status_id=active_status.id   # 🔥 THIS WAS MISSING
        )

        db.add(admin_user)
        db.flush()

        # 3️⃣ Assign Organisation Role
        DEFAULT_ORG_ADMIN_ROLE_ID = "f7129eb0-7305-4279-8994-ee9256f91447"

        db.add(UserRole(
            id=str(uuid.uuid4()),
            user_id=admin_user.id,
            role_id=DEFAULT_ORG_ADMIN_ROLE_ID,
            organisation_id=org.id   # 🔥 REQUIRED
        ))
    ...
    # -------------------------------------------------
    @staticmethod
    def update_organisation(org_id: str, org_data: Dict, db: Session):
    ...
    def get_organisation_stats(db: Session) -> Dict:
        ...
        total = db.query(Organisation).count()

        active_status = db.query(Status).filter(Status.code == "ACTIVE").first()
        inactive_status = db.query(Status).filter(Status.code == "INACTIVE").first()

        active = db.query(Organisation).filter(
            Organisation.status_id == active_status.id
        ).count() if active_status else 0

        inactive = db.query(Organisation).filter(
            Organisation.status_id == inactive_status.id
