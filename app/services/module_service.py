...
class ModuleService:
    ...
    def get_user_modules(email: str, db: Session, role_id: str, organisation_id: str):
        ...
        module_rows = (
            ...
            .join(RoleModule, Module.id == RoleModule.module_id)
        ...
        sub_rows = (
            db.query(
                Module.id.label("module_id"),
                Submodule.id.label("sub_id"),
                Submodule.name.label("sub_name"),
                Submodule.label.label("sub_label"),
                Submodule.route_key,
                Submodule.display_order,
                Privilege.name.label("privilege_name"),
            )
            .join(Submodule, Module.id == Submodule.module_id)
            .join(RoleSubmodule, Submodule.id == RoleSubmodule.submodule_id)
        ...
        for r in sub_rows:
            ...
            sub = next((s for s in sub_list if s["id"] == r.sub_id), None)
        ...
        if organisation_id and user.is_superuser:
            modules = [m for m in modules if m["name"].lower() != "organisation"]
        for m in modules:
            m["submodules"].sort(key=lambda x: x["display_order"])
