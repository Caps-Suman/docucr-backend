...
def upgrade() -> None:
    ...
    if inspector:
        ...
        # In offline mode (fresh schema), we definitely need to add it
        op.add_column('sop', sa.Column('workflow_status_id', sa.Integer(), nullable=True), schema='docucr')
        op.create_foreign_key(None, 'sop', 'status', ['workflow_status_id'], ['id'], source_schema='docucr', referent_schema='docucr')

    # Guarded drops for is_active
    user_cols = [c['name'] for c in inspector.get_columns('user', schema='docucr')] if inspector else []
    role_cols = [c['name'] for c in inspector.get_columns('role', schema='docucr')] if inspector else []
